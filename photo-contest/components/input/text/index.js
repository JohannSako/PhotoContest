    ...
    return (
        <div className="relative w-[343px] h-[50px] flex-shrink-0">
            <input
                ...
                value={value}
                onChange={onChange}
                className="w-full h-full bg-white-6 border border-gray-02 placeholder-placeholder text-text-black px-3 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-02 input-placeholder"
            />
            {type == "password" ? <span
            ...
            >
                {showPassword ? dictionary.show : dictionary.hide}
            </span> : <></>}
        </div>
    );
}